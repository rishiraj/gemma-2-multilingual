...
def main(new_tokenizer_path, extended_tokenizer_save_path):
    original_tokenizer_path = hf_hub_download(repo_id="unsloth/gemma-2-9b-it", filename="tokenizer.model", local_dir="original_tokenizer")
    ...
    tokenizer = GemmaTokenizer(vocab_file=os.path.join(extended_tokenizer_save_path, "tokenizer.model"), legacy=False)
    tokenizer.save_pretrained(extended_tokenizer_save_path)
    print(f"Tokenizer saved to {extended_tokenizer_save_path}")

    # Verify that the extended tokenizer's English vocab matches with that of the original Gemma tokenizer
    tok1 = GemmaTokenizer.from_pretrained('unsloth/gemma-2-9b-it')
    tok2 = GemmaTokenizer.from_pretrained(extended_tokenizer_save_path)
